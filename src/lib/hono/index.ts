// Hono middleware barrel (docs/SPECv2.md §5.1, §5.5, §5.9). `cloudflareAccess` and its
// `AuthVariables`/`CloudflareToolkitVariables` types are populated in a later issue. There is
// deliberately no combined/coordinator middleware exported — each piece is wired independently
// by the consumer (docs/SPECv2.md §5.5).
...
export type { LoggerVariables } from "./types.js";
