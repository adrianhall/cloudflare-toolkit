...
// return value. Imports from the public barrel, matching how a consumer would use
...
describe("sqlCount", () => {
  ...
  it("throws a NullError when row is a non-object value", () => {
    expect(() => sqlCount("not an object")).toThrow(NullError);
    expect(() => sqlCount(42)).toThrow(NullError);
  });

  it("throws a NullError when row is an object missing countProperty", () => {
    expect(() => sqlCount({ other: 5 })).toThrow(NullError);
  });

  it("throws a NullError when countProperty is present but non-numeric", () => {
    expect(() => sqlCount({ count: "5" })).toThrow(NullError);
  });

  it("supports a custom countProperty argument", () => {
    expect(sqlCount({ total: 12 }, "total")).toBe(12);
    expect(() => sqlCount({ count: 12 }, "total")).toThrow(NullError);
  });
});
