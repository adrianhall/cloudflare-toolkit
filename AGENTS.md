...
(binding per `docs/specs/SPECv2.md` §7.3):
...
3. **Ignore the line — last resort only.** If, and only if, the test environment makes a line
   ...
   self-granted exception) before merging it, per `docs/specs/SPECv2.md` §8 rule 10.
...
docs/                              # VitePress + TypeDoc documentation site (§2.4, §6.1) — its OWN
                                    ...
                                    # (repo root) or `npm run build` from inside docs/.
  index.md                         # home page
  ...
  guides/                          # one guide per functional area (§6.1); index.md is the overview
    ...
    testing.md vite-vitest.md
                                    # vite-vitest.md intentionally duplicates — not just links to —
                                    # the Agent Skill's own "Vite + Vitest configuration" section; see
                                    # the architectural rule below.
  changelog.md                     # links out to root CHANGELOG.md / GitHub Releases, not duplicated
  reference/                       # generated by TypeDoc from source JSDoc on every docs build —
                                    # gitignored, never committed (same as root dist/)
  specs/                           # THIS repo's own engineering spec + review notes (SPECv2.md,
                                    # IDEA.md) — contributor-facing planning docs, NOT published-site
                                    # content (excluded via .vitepress/config.ts's srcExclude)
  .vitepress/config.ts             # site config: nav, sidebar, srcExclude
  package.json package-lock.json typedoc.json tsconfig.json
AGENTS.md                          # this file
```
...
- **No combined/coordinator middleware.** `cloudflareAccess`, `cloudflareLogger`,
  ...
  (`docs/specs/SPECv2.md` §5.5). Do not add a bundling helper that wires more than one of them together.
- **`auth-internal` is a single audited unit** (`docs/specs/SPECv2.md` §9): a fix to JWT verification,
...
- **The Vite + Vitest worked example is deliberately duplicated, not single-sourced, between
  ...
  section and `docs/guides/vite-vitest.md`.** This is an intentional exception to "share content
  rather than forking it" (`docs/specs/SPECv2.md` §5.8): the two files serve different audiences
  (a coding agent reading `SKILL.md` in-editor vs. a human reading the published docs site), and
  each is cross-linked to the other rather than one deferring to the other for the actual code.
  Treat any change to the `wrangler.jsonc`/`vite.config.ts`/`vitest.config.ts`/test-file example in
  either file as touching both — update them together, the same way `auth-internal` changes touch
  both of its consumers above.
