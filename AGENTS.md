...
docs/                              # VitePress + TypeDoc documentation site (§2.4, §6.1) — its OWN
  ...
  guides/                          # one guide per functional area (§6.1); index.md is the overview
    ...
    testing.md
                                    ...
                                    # Skill's own section of the same name; see the architectural
                                    # rule below.
...
- **The Vite + Vitest worked example is deliberately duplicated, not single-sourced, between
  ...
  exception to "share content rather than forking it" (`docs/specs/SPECv2.md` §5.8): the two files
  serve different audiences (a coding agent reading `SKILL.md` in-editor vs. a human reading the
  published docs site), and each is cross-linked to the other rather than one deferring to the
  other for the actual code. Treat any change to the
  `wrangler.jsonc`/`vite.config.ts`/`vitest.config.ts`/test-file example in either file as touching
  both — update them together, the same way `auth-internal` changes touch both of its consumers
  above. (`docs/guides/vite-vitest.md`, a separate standalone guide that used to hold this docs
  side of the pairing, was folded into `testing.md` and removed — see issue #145.)
