...
- `sqlCount(row: unknown, countProperty: string = 'count'): number`
  ...
  For the `SELECT COUNT(*) AS count FROM t` → `.first<{count:number}>()` D1 pattern. Validates that `value` is a non-null object with a numeric `countProperty`; throws `NullError` (via `throwIfNull`) if not, since the whole point is "this should never happen — if it does, that's a bug, not a 0".
